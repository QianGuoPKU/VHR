...
def main():
    ...
    def preprocess_multilabel_function(examples):
        ...
        if "structure_features" in examples:
            result["features"] = list(map(lambda l:Translate_feature(l), examples['structure_features']))
    ...
    if args.feature_type == 'VIV_gf':
        config.hierarchy_classes = [3, 5, 14, 21, 20, 16]  
    elif args.feature_type == 'VIV_pf':
        config.hierarchy_classes = [3, 5, 14, 21, 20, 16]     
    
    elif args.feature_type == 'Human_struct':
...
#             logger.info(f"Sample {index} of the training set: {train_dataset[index]}.")
    ...
    if args.do_train:
        ...
        f.write(f'Model_{args.cv_num}_{args.per_device_train_batch_size}_{args.learning_rate}_{args.dropout_prob}:\t')
